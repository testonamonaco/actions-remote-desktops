...
echo IP:
tasklist | find /i "ngrok.exe" > nul && curl -s localhost:4040/api/tunnels | jq -r '.tunnels[0] | select(.name == "MY_TUNNEL_NAME") | .public_url' || echo "Failed to retreive NGROK authtoken - check again your authtoken"
