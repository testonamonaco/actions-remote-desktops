...
Expand-Archive -Path 'Global Mapper 22.0.zip' -DestinationPath '%HOMEDRIVE%\GM' -Verbose
& '%HOMEDRIVE%\global_mapper-22.exe' /s
...
# Copy-Item "%HOMEDRIVE%\GM\Crack\global_mapper.exe" -Destination "%PROGRAMFILES%\GlobalMapper22.0_64bit"
# Copy-Item -Path "%HOMEDRIVE%\GM\Crack\ProgramData" -Destination "C:\" -Recurse